...
def train(model, NUM_EPOCHS, optimizer, DEVICE, train_loader, valid_loader, test_loader, args, start_epoch=0, best_val_acc=0):
    ...
    if args.type == 'margin':
        ...
        model_name = config.baseline_path_erm
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        with torch.set_grad_enabled(False): # save memory during inference
            ...
            if best_val < overall_acc:
                ...
                if args.type == 'margin':
                    save_state_dict(model.state_dict(), os.path.join('./', config.margin_path_erm))
                elif args.type == 'baseline':
                    save_state_dict(model.state_dict(), os.path.join('./', config.baseline_path_erm))
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        if args.type == 'baseline':
            eval(model, valid_loader, config.baseline_path_erm)
        else:
            eval(model, valid_loader, config.margin_path_erm)
        ...
        if args.type == 'baseline':
            eval(model, test_loader, config.baseline_path_erm)
        else:
            eval(model, test_loader, config.margin_path_erm)
