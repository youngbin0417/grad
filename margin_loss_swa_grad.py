...
def train(model, NUM_EPOCHS, optimizer, DEVICE, train_loader, valid_loader, test_loader, args, start_epoch=0, best_val_acc=0):
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        with torch.set_grad_enabled(False): # save memory during inference
            ...
            # Save SWA model for ensemble (if SWA is active)
            if args.swa and epoch >= swa_start_epoch:
                epoch_path = f'ensemble_model/swa_{args.type}_epoch{epoch}.pt'
                save_state_dict(model.state_dict(), epoch_path)
            elif not args.swa:  # Save regular model for ensemble when not using SWA
                epoch_path = f'ensemble_model/swa_{args.type}_epoch{epoch}.pt'
                save_state_dict(model.state_dict(), epoch_path)
            
            # Record validation scores for ensemble
            val_acc_list.append(val_acc if isinstance(val_acc, float) else val_acc.cpu().item())
            val_worst_list.append(float(val_worst))
            save_path_acc = f'ensemble_model/swa_{args.type}_val_accs.npy'
            save_path_worst = f'ensemble_model/swa_{args.type}_val_worsts.npy'
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        if args.type == 'baseline':
            # Load ensemble paths for baseline
            ckpt_paths = [f'ensemble_model/swa_baseline_epoch{i}.pt' for i in range(config.base_epochs)]
            ...
            # Load ensemble paths for margin
            ckpt_paths = [f'ensemble_model/swa_margin_epoch{i}.pt' for i in range(config.base_epochs)]
        ...
        val_accs_path = f'ensemble_model/swa_{args.type}_val_accs.npy'
        val_worst_path = f'ensemble_model/swa_{args.type}_val_worsts.npy'
